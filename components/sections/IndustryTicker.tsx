...
const IndustryTicker: React.FC = () => {
    ...
    return (
        <section className="py-16 bg-brand-bg-secondary border-b border-gray-200 overflow-hidden relative select-none">
            <div className="absolute top-0 left-0 h-full w-12 md:w-24 bg-gradient-to-r from-brand-bg-secondary to-transparent z-10 pointer-events-none"></div>
            <div className="absolute top-0 right-0 h-full w-12 md:w-24 bg-gradient-to-l from-brand-bg-secondary to-transparent z-10 pointer-events-none"></div>
            ...
            >
                {industries.map((industry, index) => (
                    <div
                        key={`${industry.industrySlug}-${index}`}
                    ...
                    >
                        ...
                        >
                            <svg viewBox="0 0 24 24" fill="none" className="w-8 h-8 md:w-10 md:h-10 text-brand-dark group-hover:text-brand-primary transition-colors duration-300">
                                {getIconPath(industry.industrySlug)}
                            </svg>
                            <span className="text-xs md:text-sm font-bold font-heading text-brand-dark uppercase tracking-wide text-center group-hover:text-brand-primary transition-colors duration-300">
                                {industry.name}
                            </span>
                        </Link>
                    </div>
