            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
                    ...
                    <div className="max-w-md">
                        <p className="text-brand-gray text-lg">{content}</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {industries.map((industry, index) => (
                        ...
                        >
                            ...
                            <h3 className="text-2xl font-bold font-heading text-brand-dark mb-3 pr-6 group-hover:text-brand-primary transition-colors">
                                {industry.industryName}
                            </h3>
                            <p className="text-gray-600 mb-6 leading-relaxed min-h-[4.5rem]">{industry.industrydesrciption}</p>

                            <div className="border-t border-gray-100 pt-6">
                                <ul className="space-y-3">
                                    {industry.subchildindustries?.map(sub => (
                                        <li key={sub.id} className="flex items-start text-sm text-gray-500">
                                            <span className="mr-2 text-brand-secondary mt-0.5">●</span>
                                            <Link href={`/industries/${industry.slug}/${sub.subchildSlug}`} className="hover:text-brand-primary transition-colors">
                                                {sub.featuresubChild}
                                            </Link>
                                        </li>
                                    ))}
